...
func run_check() -> void:
	...
	check(state.node_state("repeat_orders") == "AVAILABLE" and state.purchase("repeat_orders", wallet)
		and wallet.balance == 9900 and state.owns("repeat_orders"), "N: Repeat Orders is now an implemented 250 Gold unlock")
	for node_id in ["trail_knowledge", "notice_board", "sleeping_guild"]:
		check(state.node_state(node_id) == "NOT YET IMPLEMENTED" and not state.purchase(node_id, wallet)
			and wallet.balance == 9900 and not state.owns(node_id), "N: Disabled " + node_id + " cannot spend or unlock")
	check(state.has_unlock_tag("repeat_expedition") and not state.has_unlock_tag("offline_progression"), "Only implemented unlock tags activate")
	...
	check(state.unlocked_ids.size() == 5 and is_equal_approx(state.global_gold_multiplier, 1.1)
